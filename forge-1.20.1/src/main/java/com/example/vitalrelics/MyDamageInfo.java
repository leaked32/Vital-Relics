...
public class MyDamageInfo
{
	 ...
	 */
	public static void directRangedAttack(final LivingEntity attacker, final float amount, final int range, final int count, final int neg_level) {
		if (count <= 0) {
			return;
		}
		...
		for (int i = 0; i < count; i += 1) {
			damages.add(
					new MyDamageInfo(
							...
							0.02f, range,  MyRangeFilter.hostileTargeted, 20, neg_level
					)
			);
		}
...
//		target.invulnerableTime = 0;
	...
	public static boolean _excluded_target(LivingEntity target) {
		...
		// return _penetration_excluded_entities.contains(target.getType().toString());
	}
	...
	public static class MyDamageTypes {
		...
		public static DamageSource myDamageSource(Level level, Entity entity) {
			...
			Holder<DamageType> baseHolder = damageTypeLookup.getOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("minecraft", "generic")));
