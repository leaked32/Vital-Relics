...
- `slowing_aura`：每20刻使`level`格内的敌对生物获得缓慢III，持续60刻。霜缚之戒默认等级10。
- `tree_feller`：等级大于0时，成功破坏原木后连锁砍倒相连原木，树叶自然凋落。林心之戒等级1。搜索限制为32格、512个额外原木；相连的原木建筑也可能被砍倒。
- `area_mining`：成功破坏方块后，挖掘其周围`level`格内任意类型的方块。采石之戒等级2；半径上限8，每次最多512个额外方块。两种采掘技能都使用真实玩家和手持工具，保留时运、精准采集、工具适用性、耐久消耗、经验和保护事件。潜行停用；更换或损坏工具停止连锁；额外破坏不会递归触发。
- `borebolt`：`speed`默认2格/刻（上限10），`durability`默认40，`durability_loss_per_tick`默认0.25，`recovery`默认0.2。剩余耐久必须严格大于方块硬度才能破坏，成功后扣除硬度。不可破坏或受保护的方块使箭消失。每刻消耗可设0；箭无法拾取，卸载或重启后消失。保留普通箭的命中被动；破坏方块可通过拥有者当前手持工具触发采掘被动。
- `compel_attack`：`range`默认24，用于选择指向目标；`search_range`默认32，用于寻找它最近的其他生物；`recovery`默认0.1。两个距离上限256。目标无视AI、阵营和近战距离立即进行一次归属于自身的攻击，施法者也可能被击中。无攻击力属性的生物造成1点伤害，否则至少1点。仍遵循正常伤害保护和事件，不修改AI目标。
