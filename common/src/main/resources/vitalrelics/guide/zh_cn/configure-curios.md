...
- `gravityless_arrows`：使佩戴者射出的所有箭矢（包括遗物法术射出的箭）不受重力影响，
     并在`level`秒后消失
...
- `no_fly_zone`：每半秒将等级数值格范围内的敌对生物移动到其下方首个碰撞表面。
  作用半径的格数等于技能等级。
...
- `healing_aura`：将每次配置的 `heal` 周期性动作共享给等级数值格范围内的
  友方生物。此技能本身不会产生治疗。
- `slowing_aura`：每20刻，使`level`格内的敌对生物获得持续60刻的缓慢III。
- `tree_feller`：等级大于0时，使用手持工具连锁砍伐相连的原木。
- `area_mining`：使用手持工具挖掘`level`格内的周边方块，并保留正常采掘条件。
...
- `return_to_bed`（返回床边）：将施法者传送至重生床旁安全的站立位置。
  若床已消失或周围被阻挡，施法将失败。
- `borebolt`（穿岩箭）：发射一支无法回收的箭，在破坏方块时消耗耐久，
  并将经过的流体方块替换为空气。
  掉落物按照使用无附魔下界合金镐采掘计算。
- `compel_attack`（强制攻击）：令最近的其他生物立即攻击一次指向的生物。
  指向的生物自身不会成为攻击者。
...
- `slowing_aura`：每20刻，使`level`格内的敌对生物获得持续60刻的缓慢III
  （效果等级2）。霜缚之戒的等级为10，因此效果会在结束前刷新。
- `tree_feller`：等级大于0时生效。玩家成功破坏原木后，会使用相同的手持工具和
  采掘条件砍伐相连原木，包括斜向连接。搜索范围限于首个原木32格内，最多额外破坏
  512个原木。树叶自然凋落。潜行可停用；更换或损坏工具会终止操作。相连的原木建筑
  也可能被砍倒。林心之戒的等级为1。
- `area_mining`：成功破坏方块后，按照距离由近至远挖掘`level`格内任意类型的
  周边方块。半径上限为8，每次最多额外破坏512个方块。它会保留手持工具、时运、
  精准采集、工具适用性、耐久消耗、经验掉落和方块破坏事件。潜行可停用；更换或损坏
  工具会终止操作。额外破坏不会递归触发新的扩展。采石之戒的等级为2。
- `borebolt`：沿施法者视线发射箭矢。`speed`默认每刻2格（上限10），
  `durability`默认40，`recovery`默认0.2。耐久不会随时间减少。
  只有剩余耐久严格大于方块硬度时才能将其破坏，成功后消耗
  等于硬度的耐久。耐久不足、不可破坏方块或交互被拒绝都会使箭消失。掉落物按照
  使用一把崭新且无附魔的下界合金镐采掘计算；不会修改施法者的手持物品或附魔。
  箭经过的流体方块会被替换为空气且不消耗耐久；含水固体方块保持不变。若流体交互
  被拒绝，箭会消失。
  箭无法拾取，卸载或重启后不会保留，并且仍可触发普通的箭矢命中被动。
- `compel_attack`：`range`默认24，用于选择指向的生物；`search_range`
  默认32，用于寻找离它最近的攻击者；`recovery`默认0.1。两个距离上限均为256。
  指向的生物自身不会成为候选攻击者，但施法者可以成为攻击者。最近的生物无需等待
  AI目标、阵营判断或正常近战距离，立即对指向的生物进行一次归属于自身的攻击。
  没有攻击力属性的生物造成1点伤害，
  其他生物至少造成1点伤害。正常伤害保护和事件仍然有效，AI目标不会被改写。
