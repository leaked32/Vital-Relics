...
- `return_to_bed`（返回床边）：将施法者传送至重生床旁安全的站立位置。
  若床已消失或周围被阻挡，施法将失败。
- `borebolt`（穿岩箭）：发射一支无法回收的箭，并在破坏方块时消耗耐久。
  掉落物按照使用无附魔下界合金镐采掘计算。
- `compel_attack`（强制攻击）：令指向的生物立即攻击一次最近的有效生物。
  被强制攻击的生物自身与施法者不会成为攻击目标。
...
- `compel_attack`：`range`默认24，用于选择指向的生物；`search_range`
  默认32，用于寻找它最近的攻击目标；`recovery`默认0.1。两个距离上限均为256。
  被强制攻击的生物自身和施法者不会成为候选目标。该生物无需等待AI目标、阵营判断或
  正常近战距离，立即进行一次归属于自身的攻击。没有攻击力属性的生物造成1点伤害，
