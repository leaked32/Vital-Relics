...
- `return_to_bed`（返回床邊）：將施法者傳送至重生床旁安全的站立位置。
  若床已消失或周圍被阻擋，施法將失敗。
- `borebolt`（穿岩箭）：發射一支無法回收的箭，並在破壞方塊時消耗耐久。
  掉落物按照使用無附魔獄髓鎬採掘計算。
- `compel_attack`（強制攻擊）：令指向的生物立即攻擊一次最近的有效生物。
  被強制攻擊的生物自身與施法者不會成為攻擊目標。
...
- `compel_attack`：`range`預設24，用於選擇指向的生物；`search_range`
  預設32，用於尋找它最近的攻擊目標；`recovery`預設0.1。兩個距離上限均為256。
  被強制攻擊的生物自身和施法者不會成為候選目標。該生物無需等待AI目標、陣營判斷或
  正常近戰距離，立即進行一次歸屬於自身的攻擊。沒有攻擊力屬性的生物造成1點傷害，
