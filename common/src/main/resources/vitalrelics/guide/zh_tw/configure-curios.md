...
- `gravityless_arrows`：使佩戴者射出的所有箭矢（包括遺物法術射出的箭）不受重力影響，
     並在`level`秒後消失
...
- `healing_aura`：將每次設定的 `heal` 週期性動作分享給等級數值格範圍內的
  友方生物。此技能本身不會產生治療。
- `slowing_aura`：每20刻，使`level`格內的敵對生物獲得持續60刻的緩慢III。
- `tree_feller`：等級大於0時，使用手持工具連鎖砍伐相連的原木。
- `area_mining`：使用手持工具挖掘`level`格內的周邊方塊，並保留正常採掘條件。
...
- `return_to_bed`（返回床邊）：將施法者傳送至重生床旁安全的站立位置。
  若床已消失或周圍被阻擋，施法將失敗。
- `borebolt`（穿岩箭）：發射一支無法回收的箭，在破壞方塊時消耗耐久，
  並將經過的流體方塊替換為空氣。
  掉落物按照使用無附魔獄髓鎬採掘計算。
- `compel_attack`（強制攻擊）：令最近的其他生物立即攻擊一次指向的生物。
  指向的生物自身不會成為攻擊者。
...
- `slowing_aura`：每20刻，使`level`格內的敵對生物獲得持續60刻的緩慢III
  （效果等級2）。霜縛之戒的等級為10，因此效果會在結束前刷新。
- `tree_feller`：等級大於0時生效。玩家成功破壞原木後，會使用相同的手持工具和
  採掘條件砍伐相連原木，包括斜向連接。搜尋範圍限於首個原木32格內，最多額外破壞
  512個原木。樹葉自然凋落。潛行可停用；更換或損壞工具會終止操作。相連的原木建築
  也可能被砍倒。林心之戒的等級為1。
- `area_mining`：成功破壞方塊後，按照距離由近至遠挖掘`level`格內任意類型的
  周邊方塊。半徑上限為8，每次最多額外破壞512個方塊。它會保留手持工具、幸運、
  絲綢之觸、工具適用性、耐久消耗、經驗掉落和方塊破壞事件。潛行可停用；更換或損壞
  工具會終止操作。額外破壞不會遞迴觸發新的擴展。採石之戒的等級為2。
- `borebolt`：沿施法者視線發射箭矢。`speed`預設每刻2格（上限10），
  `durability`預設40，`recovery`預設0.2。耐久不會隨時間減少。
  只有剩餘耐久嚴格大於方塊硬度時才能將其破壞，成功後消耗
  等於硬度的耐久。耐久不足、不可破壞方塊或互動被拒絕都會使箭消失。掉落物按照
  使用一把嶄新且無附魔的獄髓鎬採掘計算；不會修改施法者的手持物品或附魔。
  箭經過的流體方塊會被替換為空氣且不消耗耐久；含水固體方塊保持不變。若流體互動
  被拒絕，箭會消失。
  箭無法拾取，卸載或重新啟動後不會保留，並且仍可觸發普通的箭矢命中被動。
- `compel_attack`：`range`預設24，用於選擇指向的生物；`search_range`
  預設32，用於尋找離它最近的攻擊者；`recovery`預設0.1。兩個距離上限均為256。
  指向的生物自身不會成為候選攻擊者，但施法者可以成為攻擊者。最近的生物無需等待
  AI目標、陣營判斷或正常近戰距離，立即對指向的生物進行一次歸屬於自身的攻擊。
  沒有攻擊力屬性的生物造成1點傷害，
  其他生物至少造成1點傷害。正常傷害保護和事件仍然有效，AI目標不會被改寫。
