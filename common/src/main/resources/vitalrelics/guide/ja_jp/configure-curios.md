...
- `borebolt`: ブロック破壊で耐久を消費し、通過した液体を空気に置き換える
  回収不能の矢を放ちます。
  ドロップはエンチャントなしのネザライトのツルハシを使った場合として計算されます。
- `compel_attack`: 狙った生物を、最寄りの別生物に即座に1回攻撃させます。
  狙った生物自身は攻撃者になれません。
...
- `borebolt`：使用者の視線方向へ矢を放つ。`speed`は既定2ブロック/ティック
  ...
  計算され、使用者の手持ち道具やエンチャントは変更しない。通過した液体ブロックは耐久を
  消費せず空気に置き換え、水没した固体ブロックは維持する。液体への操作が拒否された場合は
  矢が消える。矢は回収できず、アンロードや
  再起動後には残らない。通常の矢命中パッシブは有効。
- `compel_attack`：`range`は狙った生物の選択距離（既定24）、`search_range`は
  その生物を攻撃する最寄りの別生物を探す距離（既定32）、`recovery`は0.1。両距離の
  上限は256。狙った生物自身は攻撃候補から除外されるが、使用者は攻撃者になり得る。
  AI目標、陣営、通常の近接射程を待たず、最寄りの生物に帰属する攻撃を狙った生物へ
  即座に1回行う。攻撃力属性がない
