...
public final class MySpellSystem {
	...
	public interface Handler {
		boolean activate(MyLivingEntity caster, Relic.Spells.Info spell);
	}
	...
	private MySpellSystem() {
		...
		*/
		register(Relic.SPELL_TELEPORT, (caster, spell) -> {
			final double distance = Math.min(Math.max(
					RelicSpells.numberParameter(spell, "range", 0.0), 0.0), 256.0);
			if (distance <= 0.0)
				return false;
			final MyVec3 destination = runtime.safeDestinationAlongLook(caster, distance);
			if (destination == null)
				return false;
		...
		register(Relic.SPELL_CURSE, (caster, spell) -> {
			final float intensity = (float) RelicSpells.numberParameter(spell, "intensity", 0.0);
		...
		register(Relic.SPELL_HEAL, (caster, spell) -> {
			final float amount = (float) Math.max(0.0,
					RelicSpells.numberParameter(spell, "amount", 0.0));
			final float ratio = (float) Math.max(0.0,
					RelicSpells.numberParameter(spell, "ratio", 0.0));
			final float healing = amount + caster.maxHealth() * ratio;
		...
		register(Relic.SPELL_HEALING_RAY, (caster, spell) -> {
			final float intensity = (float) Math.max(0.0,
		...
		register(Relic.SPELL_CLEANSE, (caster, spell) -> {
			if (!MyUtils.cleanseEffects(caster, MyLivingEntity.MyEffectCategory.NEGATIVE))
				return false;
		...
		register(Relic.SPELL_DASH, (caster, spell) -> {
			final double strength = Math.max(0.0,
					RelicSpells.numberParameter(spell, "strength", 0.0));
			final double vertical = RelicSpells.numberParameter(spell, "vertical", 0.0);
			if (strength <= 0.0)
				return false;
			...
			caster.setVelocity(lookX * inverseLength * strength, vertical,
		...
		register(Relic.SPELL_ARC_BURST, (caster, spell) -> {
			final float intensity = (float) Math.max(0.0,
					RelicSpells.numberParameter(spell, "intensity", 0.0));
			final int range = (int) Math.min(256, Math.max(0, Math.round(
					RelicSpells.numberParameter(spell, "range", 0.0))));
			final int count = (int) Math.min(64, Math.max(0, Math.round(
					RelicSpells.numberParameter(spell, "count", 1.0))));
			final int weaken = (int) Math.min(255, Math.max(0, Math.round(
			...
			if (damage <= 0.0F)
				return false;
			MyExtraDamageInfo.directRangedAttack(caster, damage, range, count, weaken);
		...
		register(Relic.SPELL_REPULSE, (caster, spell) -> {
			...
			final double strength = Math.max(0.0,
					RelicSpells.numberParameter(spell, "strength", 0.0));
			final double vertical = Math.max(0.0,
			...
			for (final MyLivingEntity target : caster.livingEntitiesInRange(range)) {
				...
				if (lengthSqr <= 1.0e-8)
					continue;
				final double inverseLength = 1.0 / Math.sqrt(lengthSqr);
				target.push(dx * inverseLength * strength, vertical,
		...
		register(Relic.SPELL_ABSORPTION, (caster, spell) -> {
			final int durationTicks = (int) Math.min(20 * 60 * 10, Math.max(1, Math.round(
		...
		register(Relic.SPELL_PHANTOM_STEP, (caster, spell) -> {
			final double range = Math.min(32.0, Math.max(0.0,
					RelicSpells.numberParameter(spell, "range", 0.0)));
			final float intensity = (float) Math.max(0.0,
					RelicSpells.numberParameter(spell, "intensity", 0.0));
			if (range <= 0.0 || intensity <= 0.0F)
				return false;
			final MyVec3 origin = new MyVec3(caster.x(), caster.y(), caster.z());
			final MyVec3 destination = runtime.safeHorizontalDestination(caster, range);
			if (destination == null)
				return false;
			final float damage = caster.attackDamage() * intensity;
		...
		register(Relic.SPELL_UPGRADE_ENCHANTED_BOOK, (caster, spell) -> {
			...
			if (!runtime.upgradeFirstEnchantment(caster, experienceCost,
					MyRuntimeUtils.EnchantmentFilter.ENCHANTMENT_BOOK_ONLY))
				return false;
		...
		register(Relic.SPELL_ENCHANTMENT_ASCENSION, (caster, spell) -> {
			final int experienceCost = Math.max(0, (int) Math.round(
					RelicSpells.numberParameter(spell, "experience_cost", 0.0)));
			if (!runtime.upgradeFirstEnchantment(caster, experienceCost,
					MyRuntimeUtils.EnchantmentFilter.ALL_ENCHANTED_ITEMS))
				return false;
			caster.playSound(MySound.ENCHANTMENT_TABLE_USE);
			return true;
		});
	...
	private void register(final String abilityId, final Handler handler) {
		if (handlers.putIfAbsent(abilityId, handler) != null)
			throw new IllegalStateException("Spell already registered: " + abilityId);
	}
	...
	public void syncSpellHud(final MyLivingEntity caster) {
		...
		if (tick < 0)
			return;
		final Map<String, Relic.Spells.Info> spells = RelicSpells.gatherSpells(runtime.gatherRelics(caster));
		final List<String> spellIds = new ArrayList<>(spells.keySet());
		final String selected = Scheduler.INSTANCE().selectedSpell(caster.uuid(), spellIds, tick);
		if (selected == null) {
	...
	private void syncSpellHud(final MyLivingEntity caster, final String spellId, final int tick) {
		final int cooldownTicks = Scheduler.INSTANCE().getSpellCooldownRemaining(
	...
	public void activate(final MyLivingEntity caster, String abilityId) {
		...
		final Map<String, Relic.Spells.Info> spells = RelicSpells.gatherSpells(runtime.gatherRelics(caster));
		final List<String> spellIds = new ArrayList<>(spells.keySet());
		if (abilityId.equals(SWITCH_SPELL_NEXT) || abilityId.equals(SWITCH_SPELL_PREVIOUS)) {
			final int direction = abilityId.equals(SWITCH_SPELL_NEXT) ? 1 : -1;
			final String selected = Scheduler.INSTANCE().selectSpell(
					caster.uuid(), spellIds, direction, tick);
			if (selected != null) {
				syncSpellHud(caster, selected, tick);
		...
		if (!abilityId.equals(CAST_SPELL))
			return;
		abilityId = Scheduler.INSTANCE().selectedSpell(caster.uuid(), spellIds, tick);
		if (abilityId == null)
			return;
		final Relic.Spells.Info spell = spells.get(abilityId);
		if (spell == null)
			return;
		final int remainingTicks = Scheduler.INSTANCE().getSpellCooldownRemaining(
				caster.uuid(), abilityId, tick);
		if (remainingTicks > 0) {
			runtime.showSpellCooldown(caster, abilityId, remainingTicks);
			return;
		}
		final Handler handler = handlers.get(abilityId);
		if (handler != null && handler.activate(caster, spell)) {
			Scheduler.INSTANCE().setSpellCooldown(caster.uuid(), abilityId, tick,
