...
public final class ArrowDurability {
    ...
    public ArrowDurability(double durability, double lossPerTick) {
        if (!Double.isFinite(durability) || durability <= 0 || !Double.isFinite(lossPerTick) || lossPerTick < 0)
            throw new IllegalArgumentException("Invalid arrow durability");
        this.remaining = durability;
        this.lossPerTick = lossPerTick;
    }
    public boolean tick() { remaining = Math.max(0, remaining - lossPerTick); return remaining > 0; }
    public boolean canBreak(double hardness) { return Double.isFinite(hardness) && hardness >= 0 && remaining > hardness; }
